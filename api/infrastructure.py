...
class Api(cdk.Construct):
    ...
    def __init__(self, scope: cdk.Construct, id: str, database: Database) -> None:
        ...
        policy = iam.ManagedPolicy.from_aws_managed_policy_name('service-role/AWSLambdaBasicExecutionRole')
        handler_role = iam.Role(self, 'HandlerRole', assumed_by=service_principal, managed_policies=[policy])
        ...
        self.chalice = Chalice(self, 'Chalice', source_dir=str(Api._RUNTIME_DIR), stage_config=chalice_stage_config)
        rest_api = self.chalice.sam_template.get_resource('RestAPI')
        rest_api.tracing_enabled = True

    @staticmethod
    def _create_chalice_stage_config(handler_role: iam.Role, database: Database):
